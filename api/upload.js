export default async function handler(req, res) {
  ...
  if (!fileName || !walletAddress) {
    return res.status(400).json({ error: 'Missing file name or wallet address' });
  }
  ...
  try {
    ...
    // Shelby endpoint: PUT /shelby/v1/blobs/{account}/{blobName}
    // account = wallet address, blobName = filename
    ...
    return res.status(shelbyRes.status).json({ error: 'Shelby upload failed', detail: errText });
